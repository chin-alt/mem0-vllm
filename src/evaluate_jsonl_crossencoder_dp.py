...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Data-parallel CrossEncoder JSONL evaluation. It splits the test file by query group, "
            "runs one single-GPU CrossEncoder worker per shard, then merges predictions and recomputes metrics."
        )
    )
    parser.add_argument("--test_file", required=True)
    parser.add_argument("--model_path", required=True)
    parser.add_argument("--output_dir", default="outputs/jsonl_crossencoder_dp_eval")
    ...
    parser.add_argument("--score_activation", choices=["sigmoid", "identity", "default"], default="sigmoid")
...
def split_records_by_group(test_file: str, num_shards: int) -> list[list[dict[str, Any]]]:
    ...
    for group_key, indices in sorted(groups.items(), key=lambda item: (-len(item[1]), item[0])):
        ...
        logger.debug("Assigned group %s with %d rows to shard %d", group_key, len(indices), shard_id)
...
def build_worker_command(
    ...
    cmd = [
        args.python_bin,
        str(PROJECT_ROOT / "src" / "evaluate_jsonl_crossencoder.py"),
        "--test_file",
        str(shard_input),
        "--model_path",
        args.model_path,
    ...
    if args.save_doc_text:
        cmd.append("--save_doc_text")
    else:
        cmd.append("--no_save_doc_text")
    return cmd
...
def read_latest_progress(path: Path) -> dict[str, Any]:
    ...
    except Exception as exc:  # pragma: no cover - best effort progress only
        logger.debug("Could not read progress file %s: %s", path, exc)
    return {}
...
def monitor_workers(processes: list[dict[str, Any]], total_records: int, poll_interval: float, show_progress: bool) -> None:
    if not show_progress:
        for item in processes:
            item["process"].wait()
        return
    with tqdm(total=total_records, desc="DP CrossEncoder scoring", unit="pair", dynamic_ncols=True, ascii=True) as progress:
        while True:
            completed = 0
            postfix_parts = []
            all_done = True
            for item in processes:
                ...
                item["last_completed"] = shard_completed
...
def run_workers(args: argparse.Namespace, devices: list[str], shards: list[list[dict[str, Any]]], output_dir: Path) -> float:
    ...
    if failed:
        ...
        raise RuntimeError("One or more data-parallel CrossEncoder workers failed:\n\n" + "\n\n".join(messages))
...
def merge_outputs(
    ...
    max_cuda_peak_allocated = max((float(row.get("cuda_peak_allocated_mib", 0.0)) for row in shard_metrics), default=0.0)
    max_cuda_peak_reserved = max((float(row.get("cuda_peak_reserved_mib", 0.0)) for row in shard_metrics), default=0.0)
    overall.update(
        {
            "backend": "crossencoder_data_parallel",
            ...
            "max_cuda_peak_allocated_mib": max_cuda_peak_allocated,
            "max_cuda_peak_reserved_mib": max_cuda_peak_reserved,
        }
    )
